    ...
    <div id="header">
      <div className="grid py-8">
        <div className="flex items-center justify-center gap-4">
          ...
          <h3 className="font-dancing text-3xl">{info.girl} &nbsp; &nbsp;</h3>
        </div>
        <div className="flex items-center justify-center gap-4 pt-3">
          <span className="line text-sm">{info.subtext}</span>
        </div>
      </div>
      <div className="main-menu hidden md:flex items-center justify-center gap-2 border-t border-b py-4">
        <div className="grid grid-cols-1 gap-x-8 gap-y-6 text-base/7 font-semibold text-white sm:grid-cols-2 md:flex lg:gap-x-10">
          {links.map((link) => (
            <a key={link.name} href={link.href} className="capitalize">
              {link.name}
            </a>
          ))}
        </div>
